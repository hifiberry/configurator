...
class SoundcardHandler:
        ...
        try:
            ...
            for card_name, attributes in SOUND_CARD_DEFINITIONS.items():  # type: ignore[union-attr]
                soundcard_info: Dict[str, Any] = {
                    ...
                    "headphone_volume_control": attributes.get("headphone_volume_control"),  # type: ignore[misc]
    ...
    def handle_set_dtoverlay(self) -> 'Union[Response, tuple[Response, int]]':
        ...
        try:
            ...
            valid_overlays: List[Any] = [attrs.get('dtoverlay') for attrs in SOUND_CARD_DEFINITIONS.values()]  # type: ignore[misc]
            if dtoverlay not in valid_overlays:
                return jsonify({
                    "status": "error",
                    "message": f"Invalid dtoverlay '{dtoverlay}'. Must be one of the supported HiFiBerry overlays.",
                    "valid_overlays": sorted(list(set(v for v in valid_overlays if v is not None)))
                }), 400  # type: ignore[return-value]
    ...
    def handle_disable_detection(self) -> 'Union[Response, tuple[Response, int]]':
        ...
        try:
            ...
            data: Dict[str, Any] = cast(Dict[str, Any], request.get_json() or {}) if request.is_json else {}
            ...
            if card_name:
                ...
                if not card_def:
                    return jsonify({
                        ...
                        "available_cards": list(SOUND_CARD_DEFINITIONS.keys())  # type: ignore[arg-type]
                    }), 400
                ...
                config.enable_overlay(dtoverlay, card_name=card_name, disable_eeprom=True)  # type: ignore[arg-type]
                config.save()
    ...
    def handle_detect_live_soundcard(self) -> 'Union[Response, tuple[Response, int]]':
        ...
        try:
            ...
            card_def: Optional[Dict[str, Any]] = SOUND_CARD_DEFINITIONS.get(card_name) if card_name else None  # type: ignore[arg-type,assignment]
    ...
    def handle_detect_soundcard(self) -> 'Union[Response, tuple[Response, int]]':
        ...
        try:
            ...
            if soundcard.name:  # type: ignore[union-attr]
                ...
                card_def: Optional[Dict[str, Any]] = SOUND_CARD_DEFINITIONS.get(soundcard.name)  # type: ignore[arg-type,assignment]
                ...
                return jsonify({
                    ...
                    "data": {
                        ...
                        "headphone_volume_control": soundcard.headphone_volume_control,  # type: ignore[union-attr]
